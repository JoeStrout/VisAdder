#!/usr/bin/env python3
"""
nn.Module wrapper for TinyAdder, for use with torchinfo and other PyTorch tooling.

Faithfully reproduces the same computation as litz_adder.py but structured as
proper nn.Module submodules so that torchinfo can introspect the architecture.
"""
import torch
...
# === Constants (same as litz_adder.py) ===
NUM_DIGITS = 10   # number of digits in numbers to add; must be 10 to work properly
...
SCALE_HEAD = 4
CANDIDATES_START = 5
DIGIT_POS_DIM = CANDIDATES_START + NUM_DIGITS
LAYER1_D_MODEL = CANDIDATES_START + NUM_DIGITS + 1
...
verbose = 0
_context_tokens = None  # set by add() for verbose labeling
...
class Layer0Attention(nn.Module):
	...
	def __init__(self):
		super().__init__()
		d = torch.float64
		self.q_bias = nn.Parameter(torch.ones(1, dtype=d))							  # 1 (broadcast)
		self.k_weight = nn.Parameter(torch.tensor(K_SPECIAL_SCORE - K_DIGIT_SCORE, dtype=d))  # 1
		self.k_bias = nn.Parameter(torch.tensor(K_DIGIT_SCORE, dtype=d))					  # 1
		self.v_w1 = nn.Parameter(torch.tensor(V_PROJ_SPECIAL / V_PROJ_SCALE, dtype=d))		# 1
		self.v_w2 = nn.Parameter(torch.tensor(V_PROJ_NEG_DOUBLE / V_PROJ_SCALE, dtype=d))	 # 1
		self.v_w3 = nn.Parameter(torch.tensor(1.0, dtype=d))								  # 1

	def forward(self, h):
		B, S, _ = h.shape
		d = torch.float64

		# Q = 1 broadcast across all heads
		...
		# Attention scores + ALiBi + causal mask
		...
		# Output: [B, heads, S, 1] -> [B, S, heads]
		out = torch.matmul(attn, v).transpose(1, 2).contiguous().view(B, S, -1)
		return out


class Layer0FFN(nn.Module):
	"""Layer 0 gated FFN: gate (1 broadcast) + up (11 values) + down (identity).
	...
	def __init__(self):
		...
		self.up_vals = nn.Parameter(torch.tensor(pv + [DIGIT_SCALE], dtype=torch.float64))  # 11
		self.gate_weight = nn.Parameter(torch.ones(1, dtype=torch.float64))				 # 1 (broadcast)

	def forward(self, h):
		B, S, _ = h.shape
		d = torch.float64

		# Gate: broadcast SCALE_DIM to 10 slots, plus DIGIT_DIM for slot 10
		gate_in = torch.zeros(B, S, NUM_DIGITS + 1, dtype=d, device=h.device)
		gate_in[..., :NUM_DIGITS] = h[..., SCALE_DIM:SCALE_DIM + 1] * self.gate_weight
		gate_in[..., NUM_DIGITS] = h[..., DIGIT_DIM]
		gate_out = F.relu(gate_in)

		# Up: COUNT_DIM broadcast * distinct per-slot values
...
class Layer1Attention(nn.Module):
	...
	def __init__(self):
		super().__init__()
		d = torch.float64
		self.v_weight = nn.Parameter(torch.tensor(FINAL_SCALE, dtype=d))   # 1
		self.v_bias = nn.Parameter(torch.tensor(GATE_BIAS_SHIFT, dtype=d)) # 1

	def forward(self, h):
		B, S, _ = h.shape
		d = torch.float64

		# Q = K = 0 -> uniform causal attention via softmax1
		q = torch.zeros(B, S, 1, dtype=d, device=h.device)
		k = torch.zeros(B, S, 1, dtype=d, device=h.device)

		# V: select DIGIT_POS_DIM, scale, add bias
...
class Layer1FFN(nn.Module):
	...
	def __init__(self):
		super().__init__()
		d = torch.float64
		self.gate_pos_scale = nn.Parameter(torch.tensor(V_SCALE, dtype=d))	# 1
		self.gate_neg_scale = nn.Parameter(torch.tensor(-V_SCALE, dtype=d))   # 1
		self.up_scale = nn.Parameter(torch.tensor(FINAL_SCALE, dtype=d))	  # 1 (broadcast)
...
class TinyAdderModule(nn.Module):
	"""
	36-parameter transformer for 10-digit addition, as nn.Module.
	...
	def forward(self, x):
		...
		# Layer 1: FFN + residual (narrow to 10 dims)
...
def build_model():
	"""Build and return the nn.Module version with metadata."""
	model = TinyAdderModule()
	metadata = {
		"name": "TinyAdder (nn.Module)",
...
def self_test(n=100, seed=42):
	...
	for _ in range(n):
		...
		if add(model, a, b) == a + b:
			correct += 1
	print(f"nn.Module self-test: {correct}/{n}")
...
def print_summary():
	...
	try:
		...
		print()
		summary(model, input_data=x, depth=2, col_names=["num_params", "output_size"])
	except ImportError:
		print("Install torchinfo for architecture summary: pip install torchinfo")
...
if __name__ == "__main__":
	...
	#print_summary()
	verbose = 2
	result = add(TinyAdderModule(), 1234, 5678)
